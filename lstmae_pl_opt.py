...
def train_anomalyDetection(config, dataset, num_epochs, num_gpus, checkpoint_dir=None):
    n_features = 9
...
def tune_anomalyDetection(data_dir, num_samples=20, num_epochs=200, gpus_per_trial=0, num_workers=8, test_size=0.2, random_seed=12):
    ...
    config = {
        "encoding_dim": tune.choice([16, 32, 64]),
        "num_hidden_layers": tune.choice([1, 2, 3]),
        "lr": tune.loguniform(1e-4, 1e-1)
    }
    
    scheduler = ASHAScheduler(max_t=num_epochs, grace_period=20, reduction_factor=2)
    
    reporter = CLIReporter(
        parameter_columns=["encoding_dim", "num_hidden_layers", "lr"],
        metric_columns=["loss", "training_iteration"],
        print_intermediate_tables=False
    )
    
    train_with_parameters = tune.with_parameters(train_anomalyDetection, 
                                                 dataset=dataset,
    ...
    analysis = tune.run(train_with_parameters,
...
analysis = tune_anomalyDetection(data_dir, num_samples=100, num_epochs=200, gpus_per_trial=1, num_workers=8)
